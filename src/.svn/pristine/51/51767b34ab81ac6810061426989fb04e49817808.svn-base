...
public class ExecutionConsumerModule extends RosiModule {
       ...
       if( moduleDirectory.exists() && moduleDirectory.isDirectory() ){
           ...
           for( int i= 0 ; i < files.length ; i++ ){
              ...
              if( ( lastAccess == null ) || ( lastAccess.longValue() != f.lastModified() ) ){
                   ...
                   _moduleFileMap.put( fileName , new Long( f.lastModified() ) ) ; 
              }
           }
       } 
   ...
   private void prepareCompilation( String compilerInput ) 
       ...
       _execution.setPrintStreams( _rosiOutput , _rosiError);
       ...
       try{
          ...
          while(true){
             ...
             try{
                ...
                //if( ( allFunctions.size() > 0 ) &&  mightBlock() ){ 
...
/*
                if( processCommand( command ) )triggerCount ++ ;
                allFunctions.addAll( processCommand( command ) ) ;
                ...
                debug("Executing 'engine' after TC="+triggerCount+";MB="+mightBlock()+";t="+timeDiff);
                debug("Executing 'engine' after TC="+triggerCount+";MB="+mightBlock());

                execute( triggerCommands ) ;
